...
If you don't provide `onAddToCart` but cart is enabled on the widget, the customer will see a `CART_HANDLER_NOT_CONFIGURED` error when they try to add to cart.

If your callback throws an error, the widget catches it and shows a `CART_ADD_FAILED` error message.
