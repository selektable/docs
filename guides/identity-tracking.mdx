...
1. Stores the user ID and traits in `localStorage`
2. Sends the identity to the Selektable server (fire-and-forget)
...
// {
//   visitorId: 'vtid_a1b2c3d4e5f67890',
//   sessionId: 'ssid_x9y8z7w6v5u43210',
//   anonymousId: 'vtid_a1b2c3d4e5f67890',
...
The SDK uses these `localStorage` and `sessionStorage` keys:

| Key | Storage | Purpose |
|-----|---------|---------|
| `selektable_visitor_id` | `localStorage` | Persistent visitor ID |
| `selektable_session_id` | `sessionStorage` | Current session ID |
| `selektable_anonymous_id` | `localStorage` | Anonymous ID (pre-identify) |
| `selektable_user_id` | `localStorage` | Identified user ID |
| `selektable_traits` | `localStorage` | User traits (JSON) |

## Privacy Considerations

- All IDs are generated client-side using `crypto.randomUUID()` (with fallback)
- No cookies are used — only `localStorage` and `sessionStorage`
