...
| **Custom Prompt Hint** | Optional context for the AI (e.g., "This is a modern furniture store") |
...
<script>
  document.getElementById('try-on-btn').addEventListener('click', function () {
    Selektable.open('widget_abc123', {
      ...
      // All product images (for image selector in widget)
      productImages: [
...
<AccordionGroup>
  ...
  <Accordion title="Add to cart shows an error">
    If cart is enabled on the widget, you must provide an `onAddToCart` callback. Without it, the widget will report a `CART_HANDLER_NOT_CONFIGURED` error.
  </Accordion>
