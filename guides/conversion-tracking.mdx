...
<Warning>
  Your API key is a secret. If you send the webhook from client-side JavaScript, the key will be visible in browser dev tools. For production use, prefer sending the webhook from your server where the key can be kept private.
</Warning>
...
There are two approaches: fire the webhook directly from the thank you page (client-side), or pass the tracking IDs through your form and send the webhook from your server.

### Option A: Client-side (thank you page)

Add this script to the page the customer sees after submitting the form. The embed script must be loaded on this page so that `Selektable.getIdentity()` can read the tracking IDs from the browser.
...
`Selektable.getIdentity()` is a browser API and is **not available on your server**. To send the webhook from your backend, you need to pass the tracking IDs from the browser through your form using hidden fields.

**Step 1:** Add hidden fields to your form that capture the tracking IDs before submission.
