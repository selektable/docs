...
Send a `POST` request to the conversion webhook with the order data and tracking IDs. This should be done server-side for reliability, but can also be done client-side.
...
// Pass visitorId and sessionId from the client to your server,
// then send the webhook from your backend
...
$visitor_id = $_POST['visitorId']; // Passed from client-side
...
```javascript Server-side (Node.js)
// Pass visitorId and sessionId from the client to your server
// via hidden form fields, then send the webhook from your backend
...
| `visitorId` | `string` | Yes | From `Selektable.getIdentity().visitorId` |
| `sessionId` | `string` | Yes | From `Selektable.getIdentity().sessionId` |
