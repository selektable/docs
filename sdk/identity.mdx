...
<ParamField path="traits" type="Record<string, unknown>">
  Optional metadata about the user. Stored in `localStorage` as JSON.
</ParamField>

**Behavior:**
- Stores `userId` and `traits` in `localStorage`
- Updates the cached `window.SelektableIdentity` object
- Sends identity update to all currently open widgets
- Makes a fire-and-forget `POST /api/widgets/identify` call to the server
...
```typescript
{
  visitorId: string;      // Persistent browser ID (e.g., "vtid_a1b2c3d4e5f67890")
  sessionId: string;      // Current session ID (e.g., "ssid_x9y8z7w6v5u43210")
...
**Behavior:**
- Overwrites the visitor ID in `localStorage`
- If no user is identified, also updates the anonymous ID
- Updates the cached identity object
